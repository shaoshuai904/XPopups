...
public class QMUIWindowInsetHelper {
    ...
    public static void apply(@NonNull final ViewGroup viewGroup) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(viewGroup, new OnApplyWindowInsetsListener() {
            ...
            public WindowInsetsCompat onApplyWindowInsets(View v, WindowInsetsCompat insets) {
                ...
                boolean needToDispatchNotchInsetChange = isNotchOfficialSupport() && sCurrentWindowInsetDepth == 0;
                if (sCurrentWindowInsetDepth == 0) {
                    ...
                    if (displayCutout != null) {
                        ...
                        v.setTag(R.id.ms_window_inset_layout_display_cutout, true);
                        insets = insets.consumeDisplayCutout();
                    } else {
                        insets = insets.consumeDisplayCutout();
                        Object lastHasNotchInfo = v.getTag(R.id.ms_window_inset_layout_display_cutout);
                        if (lastHasNotchInfo != null && (Boolean) lastHasNotchInfo) {
                            needToDispatchNotchInsetChange = true;
                            v.setTag(R.id.ms_window_inset_layout_display_cutout, false);
                        }
                    }
                }
    ...
    public static boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= DensityUtils.dp2px(viewGroup.getContext(), KEYBOARD_HEIGHT_BOUNDARY_DP)
//                && shouldInterceptKeyboardInset(viewGroup)
            ...
            viewGroup.setTag(R.id.ms_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
            insets.bottom = 0;
        } else {
            viewGroup.setTag(R.id.ms_window_inset_keyboard_area_consumer, null);
            if (viewGroup instanceof IWindowInsetKeyboardConsumer) {
    ...
    public static WindowInsetsCompat defaultApplySystemWindowInsets21(ViewGroup viewGroup, WindowInsetsCompat insets) {
        ...
        if (insets.getSystemWindowInsetBottom() >= DensityUtils.dp2px(viewGroup.getContext(), KEYBOARD_HEIGHT_BOUNDARY_DP)
//                && shouldInterceptKeyboardInset(viewGroup)
        ) {
            showKeyboard = true;
            if (viewGroup instanceof IWindowInsetKeyboardConsumer) {
                ...
                setPaddingBottom(viewGroup, insets.getSystemWindowInsetBottom());
            }
            viewGroup.setTag(R.id.ms_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
        } else {
            if (viewGroup instanceof IWindowInsetKeyboardConsumer) {
...
//                && view.getClass().getAnnotation(InterceptNotchInsetDispatch.class) == null
        ) {
